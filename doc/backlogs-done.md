...
Archived from `doc/backlog.md` to preserve completed phase history.
...
9. [x] Complete Stage D observability (`start/stop/error` events + optional response cache)
   ...
   - Delivered:
     - global lifecycle telemetry registry in `ferment.telemetry` with structured events (`:component`, `:transition`, `:details`, timestamp, counters),
     - lifecycle event emission wired into `app`, `runtime`, `http`, `model`, and `session` start/stop/error paths,
     - diagnostics snapshot (`/diag/telemetry`) now includes `:lifecycle` branch with totals, per-component transitions, and event window,
     - optional `/v1/act` response cache added under HTTP config (`:response-cache`) with intent-aware key, TTL, bounded size, and cache safety guards (cache only canonical `:value`, no stream),
     - cache invalidation on session mutations in `/v1/session` (`open/thaw/freeze/put/del`) scoped by `session/id`,
     - cache metrics included in telemetry (`:act/:cache` counters + KPI `:cache-hit-rate`),
     - response cache defaults to disabled in `resources/config/common/prod/http.edn` and is covered by HTTP regression tests.

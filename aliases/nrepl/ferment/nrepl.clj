(ns ^{:clojure.tools.namespace.repl/load false} ferment.nrepl
  (:require
...
(defn- configured-port
  ^Long []
  (or (parse-long* (System/getProperty "ferment.nrepl.port"))
      (parse-long* (System/getProperty "nrepl.port"))
      (parse-long* (System/getenv "ferment_NREPL_PORT"))
      (parse-long* (System/getenv "NREPL_PORT"))
      default-port))

(defn- configured-port-file
  ^String []
  (or (not-empty (System/getProperty "ferment.nrepl.port-file"))
  ...
  ([{:keys [port port-file]
   ...
   (or @!server
       (let [server (start-server :port (long port) :handler (nrepl-handler))
         ...
         (println (str "[ferment] nREPL client can be connected to port " (:port server)
                       " (" (.getPath pf) ")"))
         (reset! !server server)))))
...
(println "[ferment] Starting nREPL server")

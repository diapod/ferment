(ns
    ...
    ^{:doc    "Locale support for ferment"
      :author "Paweł Wilk"
      :added  "1.0.0"}
...
(def ^{:added "1.0.0"
  ...
  (assoc
   (into #::{}
         (map
          (juxt #(keyword "ferment.locale" (str %)) identity)
          (filter #(seq (str %))
                  l/available-locales)))
   ::default
   default-val))
...
(defn valid-arg?
  ...
  keywords it only checks if they are in the locale map (`ferment.locale/all`)."
   ...
   (if (nil? locale-specification) true
       (or
        (and (keyword? locale-specification)
             (contains? all (utils/ns-infer "ferment.locale" locale-specification use-infer)))
...
(defn strictly-valid-arg?
  ...
  in the locale map (`ferment.locale/all`). If the key is not there, it returns
  ...
  ([^java.util.Locale locale-specification
   ...
   (if (instance? java.util.Locale locale-specification) locale-specification
       (if-not (keyword? locale-specification) (l/locale locale-specification)
               (if-some [lready (all-arg (utils/ns-infer "ferment.locale" locale-specification use-infer))]
                 lready
